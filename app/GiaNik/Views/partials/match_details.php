...
<div class="flex flex-col gap-4">
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <?php else: ?>
            ...
            $filteredEvents = array_filter($events, fn($e) => in_array(strtolower($e['type']), $importantTypes));
            ...
            foreach ($displayEvents as $e):
                ...
                if ($type === 'card') {
                    $icon = '🟨';
                    if (strpos($detail, 'red') !== false) $icon = '🟥';
            ...
            <div class="flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded-lg border border-white/5 cursor-pointer hover:bg-white/10 transition-all"
                 hx-get="/api/gianik/player-details?playerId=<?php echo $e['player']['id']; ?>&fixtureId=<?php echo $fixtureId; ?>"
        ...
        <div class="flex items-center gap-4">
            ...
            foreach ($stats as $s) {
                ...
                foreach ($s['statistics'] as $stat) {
                    if ($stat['type'] === 'Ball Possession') $possession = $stat['value'];
                }
