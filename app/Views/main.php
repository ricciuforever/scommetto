...
<body class="bg-slate-50 dark:bg-darkbg text-slate-900 dark:text-slate-100 min-h-screen transition-colors duration-300 font-sans">
    ...
    <header class="sticky top-0 z-50 glass border-b border-white/10 px-6 py-4 flex justify-between items-center mb-8 text-white">
        ...
        <div class="flex items-center gap-4">
            <div class="hidden md:flex flex-col items-end">
                ...
                <div class="text-sm font-black"><span id="usage-val" class="text-accent">...</span> / 7500 <span class="text-slate-500">Credits</span></div>
            </div>
    ...
    <main class="max-w-7xl mx-auto p-6">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-10">
            ...
            <div class="glass p-5 rounded-3xl border-white/5 relative overflow-hidden group">
                <div class="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity text-success">
            ...
            <aside class="space-y-8">
                <div>
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button id="sync-btn" class="text-accent hover:text-white hover:bg-accent p-2 rounded-xl transition-all border border-accent/20">
                            <i data-lucide="refresh-cw" class="w-4 h-4"></i>
                        </button>
                    </div>
                    <div class="glass rounded-[32px] border-white/5 divide-y divide-white/5 overflow-hidden" id="history-container">
                    </div>
                </div>
                <div class="bg-gradient-to-br from-indigo-600 to-accent p-6 rounded-[32px] text-white shadow-xl shadow-indigo-500/20">
                    ...
                    <p class="text-white/80 text-sm leading-relaxed mb-4 font-medium">L'algoritmo analizza migliaia di dati per darti i consigli migliori in tempo reale.</p>
                    <button class="w-full bg-white text-indigo-600 font-black py-3 rounded-2xl text-xs uppercase tracking-widest hover:scale-105 transition-transform">Scopri di più</button>
                </div>
            </aside>
        </div>
    </main>

    <div id="analysis-modal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl">
        <div class="bg-slate-900 w-full max-w-2xl rounded-[40px] border border-white/10 shadow-2xl overflow-hidden relative">
            <button onclick="closeModal()" class="absolute top-6 right-6 w-12 h-12 rounded-2xl bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all text-white">
                <i data-lucide="x" class="w-6 h-6"></i>
            </button>
            <div class="p-10">
                <h3 id="modal-title" class="text-3xl font-black mb-8 tracking-tight text-white">Analisi Prossima Giocata</h3>
                <div id="modal-body" class="mb-10 text-slate-400 leading-relaxed font-medium">
                    Auto-Generating intelligence...
                </div>
                <div class="flex justify-end items-center gap-4">
                    <button onclick="closeModal()" class="px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-500 hover:text-white transition-colors">Chiudi</button>
                    <button id="place-bet-btn" class="hidden bg-accent hover:bg-sky-500 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-accent/30 hover:scale-105">Conferma Giocata</button>
                </div>
            </div>
        </div>
    </div>
