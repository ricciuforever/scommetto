...
<?php
// Filters are now pre-aggregated in MatchController::dashboard()
// $availableCountries: Array of unique country names
// $availableLeagues: Array of [league_id => ['name' => ..., 'country' => ...]]
        ...
        <div id="live-matches-grid" class="space-y-6">
            ...
            <?php else: ?>
                <?php foreach ($liveMatches as $match):
                    ...
                    $period = '';
                    if (in_array($statusShort, ['1H', 'HT', '2H', 'ET', 'P'])) $period = $statusShort;
                    else if ($elapsed <= 45) $period = '1H';
                    else $period = '2H';

                    $events = $match['events'] ?? [];
                    usort($events, function ($a, $b) { return $b['time']['elapsed'] - $a['time']['elapsed']; });
                    $recentEvents = array_slice($events, 0, 3);
                    ?>

                    <div class="glass rounded-[40px] p-8 border-white/5 hover:border-accent/30 transition-all group cursor-pointer relative overflow-hidden <?php echo $isPinned ? 'pinned-match' : ''; ?>"
                        ...
                        <div class="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                            ...
                            <div class="flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full border border-white/10">
                                <span class="text-[10px] font-black text-accent uppercase tracking-widest"><?php echo $period; ?></span>
                                <div class="h-3 w-px bg-white/10"></div>
                                <span class="text-[10px] font-black text-white uppercase tracking-widest"><?php echo $elapsed; ?>'</span>
                        ...
                        <div class="flex items-center justify-between gap-4 mb-8">
                            <div class="flex flex-col items-center gap-4 flex-1 group/team cursor-pointer" onclick="event.stopPropagation(); navigate('team', '<?php echo $match['teams']['home']['id']; ?>')">
                                <div class="w-16 h-16 p-2 glass rounded-2xl flex items-center justify-center group-hover/team:border-accent/50 transition-all relative">
                                    <img src="<?php echo $match['teams']['home']['logo']; ?>" class="w-full h-full object-contain drop-shadow-2xl">
                                    <div class="absolute -bottom-2 bg-slate-900 border border-white/10 px-2 rounded text-[8px] font-black uppercase text-slate-500 group-hover/team:text-accent transition-colors">Profilo</div>
                                </div>
                                <span class="text-xs font-black uppercase tracking-tight text-center leading-tight flex items-center gap-2"><?php echo htmlspecialchars($homeName); ?> <i data-lucide="chevron-right" class="w-3 h-3 text-slate-600 shrink-0"></i></span>
                            </div>

                            <div class="text-5xl md:text-6xl font-black italic tracking-tighter text-white tabular-nums flex flex-col items-center">
                                <span><?php echo $scoreHome; ?> - <?php echo $scoreAway; ?></span>
                                <span class="text-[9px] font-bold text-slate-500 tracking-widest uppercase mt-2 opacity-50">Live Score</span>
                            </div>

                            <div class="flex flex-col items-center gap-4 flex-1 group/team cursor-pointer" onclick="event.stopPropagation(); navigate('team', '<?php echo $match['teams']['away']['id']; ?>')">
                                <div class="w-16 h-16 p-2 glass rounded-2xl flex items-center justify-center group-hover/team:border-accent/50 transition-all relative">
                                    <img src="<?php echo $match['teams']['away']['logo']; ?>" class="w-full h-full object-contain drop-shadow-2xl">
                                    <div class="absolute -bottom-2 bg-slate-900 border border-white/10 px-2 rounded text-[8px] font-black uppercase text-slate-500 group-hover/team:text-accent transition-colors">Profilo</div>
                                </div>
                                <span class="text-xs font-black uppercase tracking-tight text-center leading-tight flex items-center gap-2"><i data-lucide="chevron-left" class="w-3 h-3 text-slate-600 shrink-0"></i> <?php echo htmlspecialchars($awayName); ?></span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col md:flex-row items-center justify-between gap-6 pt-6 border-t border-white/5">
                                ...
                                <?php if (!empty($recentEvents)): ?>
                                    <div class="flex items-center gap-3 overflow-x-auto no-scrollbar mask-linear-fade pr-4">
                                        <?php foreach ($recentEvents as $ev):
                                            $icon = 'info'; $color = 'slate-500';
                                            if ($ev['type'] === 'Goal') { $icon = 'trophy'; $color = 'text-accent'; }
                                            elseif ($ev['type'] === 'Card' && $ev['detail'] === 'Yellow Card') { $icon = 'alert-triangle'; $color = 'text-warning'; }
                                            elseif ($ev['type'] === 'Card') { $icon = 'alert-octagon'; $color = 'text-danger'; }
                                            ?>
                                            <div class="flex items-center gap-1.5 shrink-0 bg-white/5 px-3 py-1.5 rounded-xl border border-white/5">
                                                <span class="text-[9px] font-black text-slate-400"><?php echo $ev['time']['elapsed']; ?>'</span>
                                                <i data-lucide="<?php echo $icon; ?>" class="w-3 h-3 <?php echo $color; ?>"></i>
                                                <span class="text-[9px] font-bold text-white uppercase truncate max-w-[80px] hover:text-accent cursor-pointer transition-colors" onclick="event.stopPropagation(); navigate('player', '<?php echo $ev['player']['id']; ?>')"><?php echo $ev['player']['name']; ?></span>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                <?php else: ?>
                            ...
                            <div class="flex items-center gap-3 shrink-0">
                                <button onclick="event.stopPropagation(); navigate('match', '<?php echo $match['fixture']['id']; ?>')" class="p-3 rounded-xl bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white border border-white/5 transition-all group/btn" title="Dettagli"><i data-lucide="arrow-right" class="w-4 h-4 group-hover/btn:translate-x-1 transition-transform"></i></button>
                                <button onclick="event.stopPropagation(); openStatsModal(<?php echo $match['fixture']['id']; ?>)" class="px-5 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white border border-white/5 transition-all font-black uppercase text-[9px] tracking-widest flex items-center gap-2"><i data-lucide="bar-chart-2" class="w-3 h-3"></i> Stats & Predictions</button>
                                <button onclick="event.stopPropagation(); analyzeMatch(<?php echo $match['fixture']['id']; ?>)" class="px-5 py-3 rounded-xl bg-accent text-white shadow-lg shadow-accent/20 hover:scale-[1.02] active:scale-95 transition-all font-black uppercase text-[9px] tracking-widest flex items-center gap-2"><i data-lucide="sparkles" class="w-3 h-3"></i> AI Analysis</button>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            ...
            <?php if (!empty($upcomingMatches)): ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php foreach ($upcomingMatches as $m): ?>
                        <div onclick="navigate('match', '<?php echo $m['fixture_id']; ?>')" class="glass p-6 rounded-3xl border-white/5 hover:border-accent/30 transition-all cursor-pointer group relative overflow-hidden h-full">
                            <div class="absolute top-0 right-0 p-4 opacity-50"><i data-lucide="calendar-clock" class="w-4 h-4 text-slate-500"></i></div>
                            <div class="flex items-center gap-2 mb-4">
                                <span class="text-[8px] font-black uppercase text-accent tracking-widest truncate max-w-[150px]"><?php echo $m['league_name']; ?></span>
                                <span class="text-[8px] font-bold text-slate-500"><?php echo date('H:i', strtotime($m['date'])); ?></span>
                            </div>

                            <div class="flex items-center justify-between gap-4">
                                <div class="flex flex-col items-center gap-2 flex-1 min-w-0">
                                    <img src="<?php echo $m['home_logo']; ?>" class="w-8 h-8 object-contain">
                                    <span class="text-[9px] font-black uppercase text-center leading-tight truncate w-full"><?php echo htmlspecialchars($m['home_name']); ?></span>
                                </div>
                                <div class="text-[10px] font-black text-slate-600 italic">VS</div>
                                <div class="flex flex-col items-center gap-2 flex-1 min-w-0">
                                    <img src="<?php echo $m['away_logo']; ?>" class="w-8 h-8 object-contain">
                                    <span class="text-[9px] font-black uppercase text-center leading-tight truncate w-full"><?php echo htmlspecialchars($m['away_name']); ?></span>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-white/5">
                                <button onclick="event.stopPropagation(); analyzeMatch(<?php echo $m['fixture_id']; ?>)" class="text-[8px] font-black uppercase tracking-widest text-accent hover:text-white transition-colors">AI Forecast</button>
                                <button onclick="event.stopPropagation(); navigate('match', <?php echo $m['fixture_id']; ?>)" class="text-[8px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors">Pronostico</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
